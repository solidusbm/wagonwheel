...
- **"Danger zone" reseed:** `/admin` has a Danger Zone panel that force-reloads `sites`/`amenities`/`reservations`/`photos` from `db/seed.sql` on an already-provisioned database (normal boot only seeds a completely empty `sites` table — see `server/lib/dbBootstrap.js`). Destructive, requires typing a confirmation phrase. This is how corrected seed data reaches the live database after a code change.
