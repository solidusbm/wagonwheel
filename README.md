...
- **Not built yet:** guest email confirmations (only the admin gets notified so far), refunds — cancelling a booking in `/admin` releases the site but does not call Square to refund a card that was actually charged — and the inbound half of third-party sync described above.
