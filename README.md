...
- **Content admin:** `/admin` → Content edits homepage/hours-page text directly (hero badge/subtitle, About heading + 3 paragraphs, hours intro — footer and detailed policy lists are still hardcoded, not yet extended to this). The admin color-theme switcher ("Styles" panel) was removed — style-gallery review now happens on the static-site demo hub instead (see below). The site stays pinned to its original "Dark Cowboy" look, with the hero/nav/accent-panel contrast fixes already baked in (see `CLAUDE.md`).
- **Admin layout:** every section below Reservations (Sites, Photos, Amenities, Content, Push notifications, RoverPass/Hipcamp sync, Danger zone) is a collapsible `<details>`, closed by default — click a heading to expand it.
...
- **Admin notifications:** when a booking is confirmed, an email fires to `ADMIN_EMAIL`. Without SMTP credentials configured it logs to the server console instead of sending, so it works out of the box in dev. `/admin` has a **Booking alert email** panel that reports the settings the server actually loaded and can send a test message, so mail setup can be checked without taking a real booking. `SMTP_FROM` defaults to `SMTP_USER` — Gmail and most relays reject a From that isn't the authenticated mailbox. `ADMIN_EMAIL` is passed straight through as the recipient list, so a comma-separated string sends the alert to several people. Guests are **not** emailed yet; they only get the confirmation screen.
