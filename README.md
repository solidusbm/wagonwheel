...
- **Park-wide amenities admin:** separately, `/admin` has a "Park amenities" panel driving the homepage's "What every site includes" grid (dog park, WiFi, military discount, etc.) -- add/rename/hide/delete entries with no code change. Known amenities keep their curated icon (`public/js/app.js` → `PARK_AMENITY_ICONS`); anything added later gets a generic checkmark icon.
